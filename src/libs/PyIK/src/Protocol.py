...
class Servo:
    ...
    def getModelNumber(self):
        ...
        try:
            ...
            if arg != 'k':
                print('Servo Error in getModelNumber <dx{0}:{1}> E{2}'.format(self.protocol, self.id,
    ...
    def getFirmwareVersion(self):
        ...
        try:
            arg = tryRead(self.serial, 1)
            if arg != 'k':
                print('Servo Error in getFirmwareVersion <dx{0}:{1}> E{2}'.format(self.protocol, self.id,
                                                                                  self.serial.readline()))
                return None
            arg = tryRead(self.serial, 4)
            val = struct.unpack('i', arg)[0]
    ...
    def getReturnDelay(self):
        ...
        try:
            arg = tryRead(self.serial, 1)
            if arg != 'k':
                print('Servo Error in getReturnDelay <dx{0}:{1}> E{2}'.format(self.protocol, self.id,
                                                                              self.serial.readline()))
                return None
            arg = tryRead(self.serial, 4)
            val = struct.unpack('i', arg)[0]
    ...
    def getCWLimit(self):
        ...
        try:
            ...
            if arg != 'k':
                print(
                    'Servo Error in getCWLimit <dx{0}:{1}> E{2}'.format(self.protocol, self.id, self.serial.readline()))
                return None
            arg = tryRead(self.serial, 4)
    ...
    def getCCWLimit(self):
        ...
        try:
            arg = tryRead(self.serial, 1)
            if arg != 'k':
                print('Servo Error in getCCWLimit <dx{0}:{1}> E{2}'.format(self.protocol, self.id,
                                                                           self.serial.readline()))
                return None
            arg = tryRead(self.serial, 4)
            val = struct.unpack('f', arg)[0]
    ...
    def getTempLimit(self):
        ...
        try:
            arg = tryRead(self.serial, 1)
            if arg != 'k':
                print('Servo Error in getTempLimit <dx{0}:{1}> E{2}'.format(self.protocol, self.id,
                                                                            self.serial.readline()))
                return None
            arg = tryRead(self.serial, 4)
            val = struct.unpack('i', arg)[0]
    ...
    def getLowVoltageLimit(self):
        ...
        try:
            arg = tryRead(self.serial, 1)
            if arg != 'k':
                print('Servo Error in getLowVoltageLimit <dx{0}:{1}> E{2}'.format(self.protocol, self.id,
                                                                                  self.serial.readline()))
                return None
            arg = tryRead(self.serial, 4)
            val = struct.unpack('f', arg)[0]
    ...
    def getHighVoltageLimit(self):
        ...
        try:
            arg = tryRead(self.serial, 1)
            if arg != 'k':
                print('Servo Error in getHighVoltageLimit <dx{0}:{1}> E{2}'.format(self.protocol, self.id,
                                                                                   self.serial.readline()))
                return None
            arg = tryRead(self.serial, 4)
            val = struct.unpack('f', arg)[0]
    ...
    def getMaxTorque(self):
        ...
        try:
            arg = tryRead(self.serial, 1)
            if arg != 'k':
                print('Servo Error in getMaxTorque <dx{0}:{1}> E{2}'.format(self.protocol, self.id,
                                                                            self.serial.readline()))
                return None
            arg = tryRead(self.serial, 4)
            val = struct.unpack('f', arg)[0]
    ...
    def getAlarmFlags(self):
        ...
        try:
            arg = tryRead(self.serial, 1)
            if arg != 'k':
                print('Servo Error in getAlarmFlags <dx{0}:{1}> E{2}'.format(self.protocol, self.id,
                                                                             self.serial.readline()))
                return None
            arg = tryRead(self.serial, 4)
            val = struct.unpack('i', arg)[0]
    ...
    def getShutdownFlags(self):
        ...
        try:
            arg = tryRead(self.serial, 1)
            if arg != 'k':
                print('Servo Error in getShutdownFlags <dx{0}:{1}> E{2}'.format(self.protocol, self.id,
                                                                                self.serial.readline()))
                return None
            arg = tryRead(self.serial, 4)
            val = struct.unpack('i', arg)[0]
    ...
    def getTorqueEnable(self):
        ...
        try:
            arg = tryRead(self.serial, 1)
            if arg != 'k':
                print('Servo Error in getTorqueEnable <dx{0}:{1}> E{2}'.format(self.protocol, self.id,
                                                                               self.serial.readline()))
                return None
            arg = tryRead(self.serial, 4)
            val = struct.unpack('i', arg)[0]
    ...
    def getVoltage(self):
        ...
        try:
            ...
            if arg != 'k':
                print(
                    'Servo Error in getVoltage <dx{0}:{1}> E{2}'.format(self.protocol, self.id, self.serial.readline()))
                return None
            arg = tryRead(self.serial, 4)
            val = struct.unpack('f', arg)[0]
            return val
    ...
    def getPosition(self):
        ...
        try:
            arg = tryRead(self.serial, 1)
            if arg != 'k':
                print('Servo Error in getPosition <dx{0}:{1}> E{2}'.format(self.protocol, self.id,
                                                                           self.serial.readline()))
                return None
            arg = tryRead(self.serial, 4)
            val = struct.unpack('f', arg)[0]
    ...
    def getTemperature(self):
        ...
        try:
            arg = tryRead(self.serial, 1)
            if arg != 'k':
                print('Servo Error in getTemperature <dx{0}:{1}> E{2}'.format(self.protocol, self.id,
                                                                              self.serial.readline()))
                return None
            arg = tryRead(self.serial, 4)
            val = struct.unpack('i', arg)[0]
    ...
    def getGoalPosition(self):
        ...
        try:
            arg = tryRead(self.serial, 1)
            if arg != 'k':
                print('Servo Error in getGoalPosition <dx{0}:{1}> E{2}'.format(self.protocol, self.id,
                                                                               self.serial.readline()))
                return None
            arg = tryRead(self.serial, 4)
            val = struct.unpack('f', arg)[0]
    ...
    def getGoalSpeed(self):
        ...
        try:
            arg = tryRead(self.serial, 1)
            if arg != 'k':
                print('Servo Error in getGoalSpeed <dx{0}:{1}> E{2}'.format(self.protocol, self.id,
                                                                            self.serial.readline()))
                return None
            arg = tryRead(self.serial, 4)
            val = struct.unpack('f', arg)[0]
    ...
    def getTorqueLimit(self):
        ...
        try:
            arg = tryRead(self.serial, 1)
            if arg != 'k':
                print('Servo Error in getTorqueLimit <dx{0}:{1}> E{2}'.format(self.protocol, self.id,
                                                                              self.serial.readline()))
                return None
            arg = tryRead(self.serial, 4)
            val = struct.unpack('f', arg)[0]
    ...
    def getCWMargin(self):
        ...
        try:
            arg = tryRead(self.serial, 1)
            if arg != 'k':
                print('Servo Error in getCWMargin <dx{0}:{1}> E{2}'.format(self.protocol, self.id,
                                                                           self.serial.readline()))
                return None
            arg = tryRead(self.serial, 4)
            val = struct.unpack('i', arg)[0]
    ...
    def getCCWMargin(self):
        ...
        try:
            arg = tryRead(self.serial, 1)
            if arg != 'k':
                print('Servo Error in getCCWMargin <dx{0}:{1}> E{2}'.format(self.protocol, self.id,
                                                                            self.serial.readline()))
                return None
            arg = tryRead(self.serial, 4)
            val = struct.unpack('i', arg)[0]
    ...
    def getCWSlope(self):
        ...
        try:
            ...
            if arg != 'k':
                print(
                    'Servo Error in getCWSlope <dx{0}:{1}> E{2}'.format(self.protocol, self.id, self.serial.readline()))
                return None
            arg = tryRead(self.serial, 4)
            val = struct.unpack('i', arg)[0]
            return val
    ...
    def getCCWSlope(self):
        ...
        try:
            arg = tryRead(self.serial, 1)
            if arg != 'k':
                print('Servo Error in getCCWSlope <dx{0}:{1}> E{2}'.format(self.protocol, self.id,
                                                                           self.serial.readline()))
                return None
            arg = tryRead(self.serial, 4)
            val = struct.unpack('i', arg)[0]
    ...
    def getMoving(self):
        ...
        try:
            ...
            if arg != 'k':
                print(
                    'Servo Error in getMoving <dx{0}:{1}> E{2}'.format(self.protocol, self.id, self.serial.readline()))
                return None
            arg = tryRead(self.serial, 4)
            val = struct.unpack('i', arg)[0]
            return bool(val)
