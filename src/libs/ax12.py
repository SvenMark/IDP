...
class Ax12:
    ...
    def setVoltageLimit(self, id, lowVolt, highVolt):
        ...
        Ax12.port.flushInput()
        checksum = (~(
                    id + Ax12.AX_VL_LENGTH + Ax12.AX_WRITE_DATA + Ax12.AX_DOWN_LIMIT_VOLTAGE + lowVolt + highVolt)) & 0xff
        outData = bytes([Ax12.AX_START])
        outData += bytes([Ax12.AX_START])
        outData += bytes([id])
    ...
    def setCompliance(self, id, cwMargin, ccwMargin, cwSlope, ccwSlope):
        ...
        Ax12.port.flushInput()
        checksum = (~(
                    id + Ax12.AX_COMPLIANCE_LENGTH + Ax12.AX_WRITE_DATA + Ax12.AX_CW_COMPLIANCE_MARGIN + cwMargin + ccwMargin + cwSlope + ccwSlope)) & 0xff
        outData = bytes([Ax12.AX_START])
        outData += bytes([Ax12.AX_START])
        outData += bytes([id])
